...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("ᴛʜɪs ʙᴜᴛᴛᴏɴ ɪs ɴᴏᴛ ғᴏʀ ʏᴏᴜ 😒", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("ʙᴀᴄᴋ", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"📃 Pages {round(int(offset)/10)+1} / {round(total/10)}", callback_data="pages")]
        )
...
async def advantage_spoll_choker(bot, query):
    ...
    if files:
        ...
        k = await query.message.edit('This Movie Not Added To Our Bot Yet,Soon We Try To Add It..👍🏻')
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await query.answer('Yo Yo! Check PM, I have sent files in pm🤠',show_alert = True)
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🎈 ɪɴғᴏ 🎈', callback_data='information'),
        ...
        buttons= [[
            InlineKeyboardButton('📢 ᴜᴘᴅᴀᴛᴇs 📢', url='https://t.me/Robert_Pattinson119'),
...
async def advantage_spell_chok(msg):
    ...
    await msg.reply('Yo Yo! {message.from_user.mention} 👋🏻, I Think You Made Something Mistake In Your Spelling..,Choose Your Requested Movie Down If Your Movie Not In Belown Buttons - Go To Google Search Correct Spelling & Send Again.. 👍🏻', reply_markup=InlineKeyboardMarkup(btn))
